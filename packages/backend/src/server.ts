...
import {
  ...
  addPlats_Ingredients_Client,
  addRecherches_Client,
  addRestaurant_Client,
  addUsers,
  deletePlats_Client,
  deletePlats_Ingredients_Client,
  getElementsPlatsAfter,
  getIngredients,
  getLastElementPlats,
  getMenus_Client,
  getPlats,
  getPlats_Client,
  getPlats_Ingredients,
  getPlats_Ingredients_Client,
  getRecherches_Client,
  ...
  try {
    ...
    for (const obj of ingredients) {
      ...
      if (ingredient == undefined || ingredient.length == 0) {
        console.log(
          "Échec de l'ajout de l'association Plat_ingrédient, Ingredient inconnu"
        );
...
fastify.post("/api/restaurants", async (request, reply) => {
  ...
  console.log(
    `Tentative d'ajout d'un nouveau resto avec NomResto: ${NomResto}`
  );
  ...
  try {
    ...
    const leresto = await getRestaurant_Client(
      { ID_restaurant: result },
      true,
      false,
      1
    );
...
fastify.post("/api/recherches", async (request, reply) => {
  ...
  console.log(
    `Tentative d'ajout d'une nouvelle recherche avec ID_Menu: ${ID_menu}`
  );
  ...
  if (
    !verifyHMACSignature("POST", "recherches", data, timestamp, clientSignature)
  ) {
    return reply
      .status(400)
      .send({ status: "error", message: "Signature invalide" });
  ...
  try {
    ...
    const larecherche = await getRecherches_Client(
      { ID_Recherche: result },
      true,
      false,
      1
...
fastify.post("/api/updates", async (request, reply) => {
  ...
  if (
    !verifyHMACSignature("POST", "updates", data, timestamp, clientSignature)
  ) {
    return reply
      .status(400)
      .send({ status: "error", message: "Signature invalide" });
...
fastify.post("/api/updates/plats", async (request, reply) => {
  ...
  try {
    const platUpdate: any[] | undefined = await getPlats_Client(
      query,
      true,
      true,
      1
