...
export default function Vote_display({ligne} : {ligne: {ID_plat: number,Certified :string, Like:number,DisLike:number,Nom_plat:string}}){
  ...
  return (<View style={styles.reco}>
            <Text style={styles.text}>{ligne.Nom_plat}{ligne.ID_plat}</Text>
              ...
              <TouchableOpacity style={styles.reco1} onPress={dislike}>
                  <AntDesign name="dislike2" size={24} color="red" /><Text>{ligne.DisLike}</Text>
              </TouchableOpacity>
            </View>
  );
}
